def Black_pawn(position, board, location):
    ...
    if(row<8):
        ...
        if(column>0):
            # Checks if the location is Uppercase(is white)
            if(board[row+1][column-1].isupper()):
                # Sets location as a possible take square
                possible_moves[row+1][column-1] = 't'
        # Checks if possible to move right
        if(column<7):
            # Checks if location is Uppercase(is white)
            if(board[row+1][column+1].isupper()):
                # Sets location as a possible take square
                possible_moves[row+1][column+1] = 't'        
